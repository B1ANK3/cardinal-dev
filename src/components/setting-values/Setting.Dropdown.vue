...
export default defineComponent({
	...
	data() {
		...
		let defVal: DropdownValue = ((
			v: string | DropdownValue
		): DropdownValue => {
			if (typeof v == 'string') {
				return {
					name: v,
		...
		return {
			...
			valChange(payload: Event) {
				...
				let selected = defopt.filter(
					v => v.value == (payload.target as HTMLSelectElement).value
				)[0]
				if (!selected) {
					console.log(
						'Unexpected dropdown value does not exist',
						selected
					)
					return
				}
